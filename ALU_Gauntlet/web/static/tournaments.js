...
function resultDialog(t,match){
  ...
  const adminOnly=(t.result_submission_mode||"player_review")==="admin_only";
  const guidance=adminOnly?"Admin result entry: the submitted winner will be recorded and the bracket advanced immediately.":"Choose the winner. Staff will verify the result before the bracket advances. You can include proof or race notes for the review team.";
  const wrap=document.createElement("div");wrap.className="club-picker-overlay";
  wrap.innerHTML='<div class="club-picker glass-panel"><span class="eyebrow">MATCH RESULT</span><h2>Submit Match Result</h2><p>'+esc(guidance)+'</p><label>Winner<select id="result-winner">'+choices+'</select></label><label>Proof URL (optional)<input id="result-proof" placeholder="https://..."></label><label>Notes (optional)<textarea id="result-notes" maxlength="500" placeholder="Score, race notes, or other details"></textarea></label><div class="club-picker-actions"><button class="qa qa-blue" id="result-cancel">Cancel</button><button class="qa qa-purple" id="result-send">Submit Result →</button></div></div>';
