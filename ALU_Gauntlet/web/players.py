...
class PlayerService:
    ...
    async def list_players(self,guild_id:str,search:str="",limit:int=50):
        ...
        if search.strip():
            t=search.strip(); q["$or"]=[{"game_id":{"$regex":t,"$options":"i"}},{"user_id":{"$regex":t,"$options":"i"}},{"username":{"$regex":t,"$options":"i"}}]
    ...
    def _safe(row,include_id=False):
        ...
        allowed={"id","guild_id","user_id","username","global_name","game_id","elo","garage_pi","season_registered","season_number","streak","career_wins","career_played","defense_locked","created_at","updated_at"}
